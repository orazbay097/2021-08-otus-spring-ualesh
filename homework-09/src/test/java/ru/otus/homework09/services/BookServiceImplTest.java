...
class BookServiceImplTest {
    ...
    void getAll() {
        ...
        assertThat(this.bookService.findAll()).hasSize(books.size());
    }
    ...
    void setAuthorId() {
        this.mockRepositoryFindById();
        val authorId = 2L;
        when(authorService.findById(authorId)).thenReturn(new AuthorDto(authorId, null, null));
    ...
    void setGenres() {
        ...
        for (long genreId : genreIds) {
            when(genreService.findById(genreId)).thenReturn(new GenreDto(genreId, null));
